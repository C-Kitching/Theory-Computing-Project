...
#Master Conditions
rM=np.array([20,0,0,0.15])
aM=np.array([5,0,0,0])
bM=np.array([-5,0,0,0])
tmax=100000  # time end
h=0.1  # step size
...
plt.figure()
plt.plot(ts/period,DifEl, label = 'Electron Displacement')
plt.plot(ts/period,DifPro, label = 'Proton Seperation')
plt.legend()
plt.title('Instability')
plt.xlabel('Time / Orbital Periods')
plt.ylabel('Displacement difference between 3 body problem \n and its limiting case')
